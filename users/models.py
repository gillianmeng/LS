...
class ProjectModule(models.Model):
    ...
    project = models.ForeignKey(Project, on_delete=models.CASCADE, related_name="modules", verbose_name="项目")
    module_type = models.CharField(max_length=20, choices=ModuleType.choices, verbose_name="模块")
    title = models.CharField(max_length=120, blank=True, verbose_name="模块标题")
    enabled = models.BooleanField(default=True, verbose_name="启用")
    sort_order = models.PositiveSmallIntegerField(default=0, verbose_name="排序")
    selected_object_ids = models.JSONField(default=list, blank=True, verbose_name="已选内容ID")
    config = models.JSONField(default=dict, blank=True, verbose_name="模块配置")
    created_at = models.DateTimeField(auto_now_add=True, verbose_name="创建时间")
    updated_at = models.DateTimeField(auto_now=True, verbose_name="更新时间")
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(fields=["project", "module_type"], name="uniq_project_module_type")
        ]

    def __str__(self):
        return f"{self.project.name} · {self.get_module_type_display()}"
